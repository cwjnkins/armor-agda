...
module parseDirectoryString where
  ...
  parseTeletexString =
    parseTLV Tag.TeletexString "teletex string" Generic.Octetstring
      (parseExactLength Dig NonNesting.TLV (tell $ here' String.++ ": length mismatch") parseOctetstring)

  parsePrintableString : Parser Dig (Logging ∘ Dec) X509.PrintableString
  parsePrintableString =
    parseTLV Tag.PrintableString "printable string" Generic.Octetstring
      (parseExactLength Dig NonNesting.TLV (tell $ here' String.++ ": length mismatch") parseOctetstring)

  parseUniversalString : Parser Dig (Logging ∘ Dec) X509.UniversalString
  parseUniversalString =
    parseTLV Tag.UniversalString "universal string" Generic.Octetstring
      (parseExactLength Dig NonNesting.TLV (tell $ here' String.++ ": length mismatch") parseOctetstring)

  parseUTF8String : Parser Dig (Logging ∘ Dec) X509.UTF8String
  parseUTF8String =
    parseTLV Tag.UTF8String "UTF8 string" Generic.Octetstring
      (parseExactLength Dig NonNesting.TLV (tell $ here' String.++ ": length mismatch") parseOctetstring)

  parseBMPString : Parser Dig (Logging ∘ Dec) X509.BMPString
  parseBMPString =
    parseTLV Tag.BMPString "BMP string" Generic.Octetstring
      (parseExactLength Dig NonNesting.TLV (tell $ here' String.++ ": length mismatch") parseOctetstring)
  ...
  parseVisibleString =
    parseTLV Tag.VisibleString "universal string" Generic.Octetstring
      (parseExactLength Dig NonNesting.TLV (tell $ here' String.++ ": length mismatch") parseOctetstring)
