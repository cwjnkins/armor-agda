...
@0 noConfusionFieldsParam
  : {@0 P' : {@0 bs : List UInt8} → OID bs → @0 List UInt8 → Set}
    ...
    → NoConfusion (AlgorithmIdentifierFields P) (AlgorithmIdentifierFields P')
noConfusionFieldsParam{P'} excl {xs₁}{ys₁}{xs₂}{ys₂} xs₁++ys₁≡xs₂++ys₂ (mkAlgIDFields{bs₁}{p} o para bs≡) (mkAlgIDFields{bs₁'}{p'} o' para' bs'≡) =
     let
       @0 ++≡ : bs₁ ++ p ++ ys₁ ≡ bs₁' ++ p' ++ ys₂
       ++≡ = begin
