...
module Aeres.Data.Unicode.UTF32.Parser where
...
runParser parseChar xs = do
  (yes (success pre₁ ._ refl (mk×ₚ (mk×ₚ (singleton bs@(b₁ ∷ b₂ ∷ b₃ ∷ b₄ ∷ []) refl) (─ refl) refl) (refl , u32₁) refl) suf₁ ps≡₁)) ←
    runParser
      (parseSigma'
        {B = λ where
          ._ (mk×ₚ (singleton (b₁ ∷ b₂ ∷ b₃ ∷ b₄ ∷ []) refl) (─ refl) refl) →
        ...
        (λ where
          (mk×ₚ (singleton (b₁ ∷ b₂ ∷ b₃ ∷ b₄ ∷ []) refl) (─ refl) refl) (mk×ₚ (singleton ._ refl) (─ refl) refl) x → x)
    ...
    where no ¬p → do
      ...
      return ∘ no $ λ where
        (success prefix@.(# 0 ∷ b₂ ∷ b₃ ∷ [ b₄ ]) read read≡ (mkUTF32Char b₂ b₃ b₄ range refl) suffix refl) → ‼
          contradiction
            (success prefix _ refl (mk×ₚ (mk×ₚ (singleton prefix refl) (─ refl) refl) (refl , range) refl) _ refl)
            ¬p
  return (yes (success pre₁ _ refl (mkUTF32Char b₂ b₃ b₄ u32₁ refl) suf₁ ps≡₁))

parse : ∀ n → Parser (Logging ∘ Dec) (ExactLength UTF32 n)
parse = parseIList (tell $ here' String.++ "parse: underflow") _ Char.nonempty Char.nonnesting parseChar
