...
@0 nonempty : ∀ {@0 A B} → @0 NonEmpty A → @0 NonEmpty B → NonEmpty (Sum A B)
nonempty ne₁ ne₂ (inj₁ x) ≡[] = contradiction ≡[] (ne₁ x)
nonempty ne₁ ne₂ (inj₂ x) ≡[] = contradiction ≡[] (ne₂ x)

@0 noconfusion : ∀ {A B C} → NoConfusion A B → NoConfusion A C → NoConfusion A (Sum B C)
noconfusion nc₁ nc₂ ++≡ a (Sum.inj₁ x) = nc₁ ++≡ a x
noconfusion nc₁ nc₂ ++≡ a (Sum.inj₂ x) = nc₂ ++≡ a x

@0 nosubstrings : ∀ {@0 A B} → @0 NoSubstrings A → @0 NoSubstrings B → NoConfusion A B → NoSubstrings (Sum A B)
...
-- TODO: rename
@0 unambiguous' : ∀ {@0 A B} → @0 Unambiguous A → @0 Unambiguous B → @0 (∀ {xs} → A xs → ¬ B xs ) → Unambiguous (Sum A B)
