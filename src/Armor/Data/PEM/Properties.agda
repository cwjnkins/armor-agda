...
Rep₁ : @0 List Char → Set
Rep₁ = &ₚ CRLHeader (&ₚ CertText CRLFooter)

equiv₁ : Equivalent Rep₁ CRL
proj₁ equiv₁ (mk&ₚ fstₚ₁ (mk&ₚ fstₚ₂ sndₚ₁ refl) refl) =
  mkCert fstₚ₁ fstₚ₂ sndₚ₁ refl
proj₂ equiv₁ (mkCert header body footer refl) =
  mk&ₚ header (mk&ₚ body footer refl) refl
...
nonempty₁ : NonEmpty CRL
nonempty₁ (mkCert (mkCRLBoundary refl eol refl) body footer ()) refl

@0 noOverlapHeaderText₁ : NoOverlap CRLHeader (&ₚ CertText CRLFooter)
noOverlapHeaderText₁ ws [] ys₁ xs₂ ys₂ ++≡ x₁ x₂ = inj₁ refl
noOverlapHeaderText₁ ws xs₁@(x₁ ∷ xs₁') ys₁ xs₂ ys₂ ++≡
  (mkCRLBoundary{b}{e} begin eol bs≡) (mkCRLBoundary{b₁}{e₁} begin₁ eol₁ bs≡₁) =
  inj₂ noway
  where
  open ≡-Reasoning
...
noOverlapTextFooter₁ : NoOverlap CertText CRLFooter
noOverlapTextFooter₁ ws [] ys₁ xs₂ ys₂ x x₁ x₂ = inj₁ refl
noOverlapTextFooter₁ ws xs₁@(x₁ ∷ xs₁') ys₁ xs₂ ys₂ ++≡
  text₁@(mkCertText{b₁}{f₁} body₁ final₁ bs≡₁)
  text₂@(mkCertText{b₂}{f₂} body₂ final₂ bs≡₂) =
  ...
  noway₁ : ¬ CRLFooter xs₂
  noway₁ (mkCRLBoundary{e = e'} refl eol bs≡) =
    contradiction{P = '-' ∈ B64.charset ++ String.toList "=\r\n"}
      (subst₀ (_∈ B64.charset ++ String.toList "=\r\n") x₁≡' x₁∈)
      (toWitnessFalse{a? = _ ∈? _} tt)
    where
...
noOverlap₁ ws xs₁@(x₁ ∷ xs₁') ys₁ xs₂ ys₂ ++≡ (mkCert{h₁}{b₁}{f₁} header₁ body₁ footer₁ bs≡₁) (mkCert{h₂}{b₂}{f₂} header₂ body₂ footer₂ bs≡₂) =
  ...
  @0 x₁≡n : x₁ ≡ '\n'
  x₁≡n = caseErased footer₁ ,′ footer₂ ret (const _) of λ where
    (mkCRLBoundary{e = e₁} refl eol₁ refl , mkCRLBoundary{e = e₂} refl eol₂ refl) → ─
      let e₂≡ : e₂ ++ xs₁ ≡ e₁
          e₂≡ = ++-cancelˡ _ _ _ ++xs₁≡
      in
      caseErased eol₁ ,′ eol₂ ret (const _) of λ where
  ...
  noway₁ : ¬ CRL xs₂
  noway₁ (mkCert{b'}{f'} (mkCRLBoundary refl eol refl) body footer bs≡) =
    contradiction{P = '-' ≡ '\n'} (trans (sym x₁≡) x₁≡n) λ ()
    where
    @0 x₁≡ : x₁ ≡ '-'
    x₁≡ = ∷-injectiveˡ (trans ++≡ (cong (_++ ys₂) bs≡))

@0 unambiguous₁ : Unambiguous CRL
unambiguous₁ = Iso.unambiguous iso₁
  (Seq.unambiguousNO (CRLBoundary.unambiguous _)
  (Seq.unambiguousNO CertText.unambiguous
                     (CRLBoundary.unambiguous _)
    noOverlapTextFooter₁) noOverlapHeaderText₁)

@0 unambiguousCRLList : Unambiguous CRLList
unambiguousCRLList = IList.unambiguousNO unambiguous₁ nonempty₁ noOverlap₁ 
