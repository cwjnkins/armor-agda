...
REASON = fstₚ (Success.value (toWitness{a? = Logging.val (runParser (parseOIDValue (length REASONLit)) REASONLit)} tt))

INVALIDITYLit : List UInt8
INVALIDITYLit =  # 85 ∷ # 29 ∷ [ # 24 ]

INVALIDITY : OIDValue INVALIDITYLit
INVALIDITY = fstₚ (Success.value (toWitness{a? = Logging.val (runParser (parseOIDValue (length INVALIDITYLit)) INVALIDITYLit)} tt))

CERTISSUERLit : List UInt8
CERTISSUERLit =  # 85 ∷ # 29 ∷ [ # 29 ]

CERTISSUER : OIDValue CERTISSUERLit
CERTISSUER = fstₚ (Success.value (toWitness{a? = Logging.val (runParser (parseOIDValue (length CERTISSUERLit)) CERTISSUERLit)} tt))
