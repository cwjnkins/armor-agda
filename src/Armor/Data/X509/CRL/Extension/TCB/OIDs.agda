...
CRLNL = fstₚ (Success.value (toWitness{Q = Logging.val (runParser (parseOIDValue (length CRLNLit)) CRLNLit)} tt))

DCRLILit : List UInt8
DCRLILit =  # 85 ∷ # 29 ∷ [ # 27 ]

DCRLI : OIDValue DCRLILit
DCRLI = fstₚ (Success.value (toWitness{Q = Logging.val (runParser (parseOIDValue (length DCRLILit)) DCRLILit)} tt))

IDPLit : List UInt8
IDPLit =  # 85 ∷ # 29 ∷ [ # 28 ]

IDP : OIDValue IDPLit
IDP = fstₚ (Success.value (toWitness{Q = Logging.val (runParser (parseOIDValue (length IDPLit)) IDPLit)} tt))

FCRLLit : List UInt8
FCRLLit =  # 85 ∷ # 29 ∷ [ # 46 ]

FCRL : OIDValue FCRLLit
FCRL = fstₚ (Success.value (toWitness{Q = Logging.val (runParser (parseOIDValue (length FCRLLit)) FCRLLit)} tt))
